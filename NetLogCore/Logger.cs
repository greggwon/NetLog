...
namespace NetLog.Logging
{
    ...
    public class Logger : ILogger
    {
		...
		protected Level MapLevel( LogLevel logLevel ) {
			Level lvl = Level.INFO;
			switch( logLevel ) {
				case LogLevel.Debug: lvl = Level.FINE; break;
				case LogLevel.Information: lvl = Level.INFO; break;
				case LogLevel.Warning: lvl = Level.WARNING; break;
				case LogLevel.Error: lvl = Level.SEVERE; break;
				case LogLevel.Trace: lvl = Level.FINER; break;
				case LogLevel.Critical: lvl = Level.SEVERE; break;
				case LogLevel.None: lvl = Level.OFF; break;
			}
			return lvl;
		}
		...
		public class LogState : IDisposable {
			public String cur;
			public Logger log;
			public LogState( Logger log, String prev ) {
				this.cur = prev;
				this.log = log;
			}
		...
		public Stack<String> scopes = new Stack<String>();
		public void Log<TState>( LogLevel logLevel, EventId eventId, TState state, Exception exception, Func<TState, Exception, string> formatter ) {
			log( MapLevel(logLevel), curScope+formatter( state, null ), exception );
		}

		public bool IsEnabled( LogLevel logLevel ) {
			return IsLoggable( MapLevel( logLevel ) );
		}

		public IDisposable BeginScope<TState>( TState state ) {
			scopes.Push( state.ToString() );
			return new LogState( this, state.ToString() );
		}
	}
}
