...
namespace NetLog.Logging
{
	...
	public class LogManager
	{
		...
		private static readonly ConcurrentDictionary<string, Handler> handlers = new ConcurrentDictionary<string, Handler>( 200, 200 );
		private static readonly ConcurrentDictionary<string, Filter> filters = new ConcurrentDictionary<string, Filter>( 200, 200 );
		private static readonly ConcurrentDictionary<string, Formatter> formatters = new ConcurrentDictionary<string, Formatter>( 200, 200 );
		...
		private void ActivateConfigurationSection( NetLogConfigurationSection sect ) {
			...
			foreach( HandlerConfiguration hnd in sect.Handlers ) {
				...
				if( formatters.ContainsKey( fmt ) ) {
					...
					formatters.TryAdd( fmt, hndlr.Formatter );
					hndlr.Formatter = formatters[ fmt ];
				}
			...
			do {
				...
				foreach( LoggerConfiguration lg in sect.Loggers ) {
					...
					Logger log;
					if( lg.ClassName == null || lg.ClassName.Length == 0 || lgs.ContainsKey( name ) ) {
						if( lgs.ContainsKey( name ) ) {
		...
		/// <summary>
		/// This is used to get the level of a Logger which has been configured using properties
		/// or which has no explicit level set.
		/// 
		/// The individual Logger instances will have a null Level by default so that parent
		/// Logger instances Level control their logging if they do not have an explicit level.
		/// </summary>
		/// <param name="name"></param>
		/// <returns></returns>
		public Level LevelOfLogger( string name ) {
			// Explicitly assigned levels from logging.properties
			Dictionary<string, Level> lvls = levels;
			if( lvls.ContainsKey( name ) ) {
				return lvls[ name ];
			}
			...
			//	"logger", "name", "more", "other"
			int idx;
			Dictionary<String, Logger> lgs = loggers;
			while( ( idx = name.LastIndexOf( '.' ) ) >= 0 ) {
				string nm = name.Substring( 0, idx );
				Logger hm;
				// This reference has to be to ".level" and not ".Level" as ".Level"
				// will recursively come back to here.
				if( lgs.ContainsKey( nm ) && (hm = lgs[ nm ]).level != null ) {
					// Return level of parent logger with an explicit level set.
					return hm.level;
				} else if( lvls.ContainsKey( nm ) ) {
					// Check for property value at this level
					return lvls[ nm ];
				}
				name = nm;
			}
			...
			// There is practically no path to this code because "" will match.
			return Level.ALL;
		}
		...
		/// <summary>
		/// Add the indicated logger if it doesn't already exist.   If it does,
		/// the past logger will not be added.
		/// </summary>
